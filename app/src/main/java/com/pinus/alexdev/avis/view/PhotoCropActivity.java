...
public class PhotoCropActivity extends AppCompatActivity {
    public static final String INTENT_URI_KEY = "imageUri";
    public static final String INTENT_ACTIVITY_NAME_KEY = "activityName";
    Intent intent;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Uri uri = Uri.parse(intent.getStringExtra(INTENT_URI_KEY));
        cropImageView.setImageUriAsync(uri);

        cropImageView.setOnCropImageCompleteListener((view, result) -> {
            Intent intent = null;
            switch (this.intent.getStringExtra(INTENT_ACTIVITY_NAME_KEY)) {
                case "ProfileActivity":
                    ProfileActivity.updatedPhotoBitmap = result.getBitmap();
                    intent = new Intent(this, ProfileActivity.class);
                    startActivity(intent);
                    break;
                case "CompanyActivity":
                    CompanyActivity.updatedPhotoBitmap = result.getBitmap();
                    intent = new Intent(this, CompanyActivity.class);
                    startActivity(intent);
                    break;
                case "AddBranchActivity":
                    AddBranchActivity.updatedPhotoBitmap = result.getBitmap();
                    break;
            }
            finish();
        });
...
//Может в дальнейшем именно исходники телеграмма помогут сделать нужное активити для кроппинга фото, пока этот код будет просто закоменчен как напоминалка, чтобы в дальнейшем попробовать сделать
